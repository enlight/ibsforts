...
function writeOutputFiles(files) {
    ...
    return Promise.resolve()
        ...
        return files.reduce(function (dirs, currentFile) {
            var dir = path.dirname(currentFile.name);
            if (dirs.indexOf(dir) < 0) {
                dirs.push(dir);
            }
            return dirs;
        }, [])
            .reduce(function (promiseChain, dir) {
            return promiseChain.then(function () { return makeDir(dir); });
        }, Promise.resolve());
    })
        .then(function () { return Promise.all(files.map(function (file) { return fs_promisified_1.writeFile(file.name, file.text, 'utf8'); })); })
